...
export const approveBreRejectedLeadManually = async (req, res) => {
    try {
        ...
        const leadId = parseInt(id);
        const employeeId = req.employee.id;
        const { calculated_loan_amount, remarks } = req.body;
...
export const fetchAllDisbursedLeadsWithDetails = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: totalLeads > 0 ? "Disbursed leads fetched successfully" : "No disbursed leads found",
            data: leads,
            pagination: {
                currentPage: page,
                totalPages: Math.ceil(totalLeads / limit),
...
export const fetchAllClosedLeadsWithDetails = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: closedLeads.length > 0 ? "Closed leads fetched successfully" : "No closed leads found",
            data: leads,
            pagination: {
                currentPage: page,
                totalPages: Math.ceil(totalLeads / limit),
...
export const fetchLeadDetailsById = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: true, message: 'Failed to fetch lead', error: error.message });
    }
};
...
export const getThirdPartyApiResponse = async (req, res) => {
    try {
        ...
        if (isNaN(leadId) || !apiType) {
            return res.status(400).json({ status: 'error', message: 'Invalid lead ID or missing apiType' });
        }
        const selectFilter = {
            api_response: true,
            created_at: true
        ...
        if (!thirdPartyLeads.length) {
            return res.status(404).json({ status: 'error', message: 'No third-party lead data found' });
        }
        ...
        res.status(500).json({ success: true, message: 'Failed to fetch third-party lead data', error: error.message });
    }
};
export const getLeadLogs = async (req, res) => {
    try {
        ...
        if (!logs.length) {
            return res.status(404).json({ status: 'error', message: 'No logs found for this lead' });
        }
        ...
        console.error("Fetch error:", error);
        res.status(500).json({ success: false, message: 'Failed to fetch lead logs', error: error.message });
    }
}
...
export const uploadMarketingLeadsInBulk = async (req, res) => {
    try {
        ...
        const missingFields = Object.keys(requiredFields).filter(field => !headers.includes(field));
        ...
        const splitName = (fullName) => {
            ...
            const m_name = nameParts.length > 2 ? nameParts.slice(1, -1).join(' ') : null;
            const l_name = nameParts.length > 1 ? nameParts[nameParts.length - 1] : null;
...
export const disbursed = asyncHandler(async (req, res) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        await tx.disbursal.update({
            ...
            data: { is_disbursed: true, utr, transaction_history_id: transactionHistory.id }
        });
